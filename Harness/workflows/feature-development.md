...
2. **Explore the design** (`how` before you build). Walk the relevant subsystems: existing components, packages, i18n keys, backend contracts. Run this as a parallel design exploration per the architecture-change workflow when the feature crosses boundaries. Skip only with a one-line `skip: <reason>`.
...
7. **Small ordered commits, then the review handoff** (`workflows/handoff.md`). The handoff stops the task: nothing is pushed, no PR is opened. You own the diff and answer review questions; the human reviews and opens the PR.
