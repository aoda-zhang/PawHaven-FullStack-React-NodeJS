...
The **orchestrator only** (`AGENT.md`) creates, appends to, and clears the task log. Subagents never write to it.
...
{agent-level dispatch plan from §3.2 of AGENT.md: who runs, in what order}
...
1. When the final summary is presented (STEP 7 of `AGENT.md`), the orchestrator **must** ask: **"是否需要清空运行log？"** (or in English: "Clear the run log? Yes / No").
...
**Related Docs**: [Agent Communication Protocol](./agent-communication-protocol.md) | [Orchestrator Agent](../AGENT.md) | [Review Handoff](../workflows/handoff.md)
