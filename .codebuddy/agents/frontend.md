...
- **Split & Sync units**: when the orchestrator dispatches you as ONE unit (e.g. `U3`) of `workflows/parallel-execution.md`, your prompt will contain the SYNC PROTOCOL — follow it exactly: implement your unit → append your status + report to today's memory file `.codebuddy/memory/YYYY-MM-DD.md` (append-only, your own unit section) → READ THE ENTIRE memory file → if all sibling units are DONE, report back; if any sibling is NOT DONE, keep re-reading the memory file until all are DONE, then report back. If your own unit FAILS, append FAILED + reason and report immediately.
...
| Doc                                                 | When           | Purpose                                                                                                              |
| --------------------------------------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------- |
| `.codebuddy/docs/PawHaven-Frontend-Architecture.md` | **Every task** | Understand feature modules, packages, routing, state management, design tokens, i18n architecture, module boundaries |
...
│     → read_file .codebuddy/docs/PawHaven-      │
