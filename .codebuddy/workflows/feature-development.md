...
4. **Delegate.** Fire a subagent per workstream with a named data shape and explicit success criteria. Review every diff yourself; write your own summary, don't pass through subagent words. **If the implementation is long** (multi-file, cross-module, likely > 300s per subagent), delegate via the **parallel-execution** workflow (`workflows/parallel-execution.md`) instead: split into small units, dispatch them in parallel, and join them through the memory-file barrier — each unit appends its status to today's `.codebuddy/memory/YYYY-MM-DD.md`, reads the entire memory file, and waits for all sibling units before reporting back.
