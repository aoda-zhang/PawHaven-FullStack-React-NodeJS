...
3. **Write the handoff summary.** In the reply, state:
   - **Doc Impact**: classify as `none` (no docs need updating), `update` (existing docs need updating), or `create` (new docs needed). This is mandatory for every handoff.
   ...
   - If Doc Impact is `update` or `create`, note which documents need updating or creating, and route to `knowledge-update` agent for permanent documentation.
