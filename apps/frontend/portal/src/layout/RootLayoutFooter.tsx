...
export const RootLayoutFooter = () => {
  ...
  const stats = [
    { emoji: '🐾', number: '2,841', label: t('footer.stats.animals_rescued') },
    { emoji: '🙌', number: '384', label: t('footer.stats.active_volunteers') },
          ...
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-4 lg:col-span-4">
            {footerColumns.map((column) => (
              <div key={column.heading}>
                <h4 className="text-footer-muted mb-4 text-xs font-semibold tracking-widest uppercase">
                  {column.heading}
                </h4>
                <ul className="flex flex-col gap-2.5">
                  {column.links.map((link) => (
                    <li key={link.label}>
                      <Link
                        to={link.to}
                        className="text-footer-text hover:text-text-inverse block text-left text-sm transition-colors"
                      >
                        {link.label}
                      </Link>
                    </li>
